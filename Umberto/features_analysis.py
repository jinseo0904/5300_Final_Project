...
def load_and_preprocess_data(df):
    ...
    df.loc[:, 'stress_normalized'] = df.groupby('id')['Q7_STRESS'].transform(lambda x: (x - x.mean()) / x.std())
    ...
    if 'stress_2' in df.columns:
        ...
        df.loc[:, 'watch_stress_normalized'] = df.groupby('id')['stress_2'].transform(lambda x: (x - x.mean()) / x.std())
...
def analyze_stress_by_subject(df):
    ...
    """
    stress_measures = ['Q7_STRESS', 'stress_2'] if 'stress_2' in df.columns else ['Q7_STRESS']
    ...
    for group_name, features in feature_groups.items():
        ...
        if available_features:
            ...
            sns.heatmap(df[available_features].corr(), 
                       ...
                       center=0,
                       fmt='.2f')
            plt.title(f'{group_name.replace("_", " ").title()} Feature Correlations')
            plt.tight_layout()
...
def identify_stress_predictors(df):
    ...
    sensor_features = ['SLEEP_MINUTES', 'SCREEN_ON_SECONDS', 'UNLOCK_EVENTS_NUM', 'MIMS_SUM_WEAR']
    ...
    for group_name, group_cols in predictor_groups.items():
        ...
        if available_cols:
            correlations = df[available_cols + ['stress_normalized']].corr()['stress_normalized'][:-1]
            grouped_correlations[group_name] = correlations.sort_values(ascending=False)
    
    feature_importance = pd.concat([corrs for corrs in grouped_correlations.values()])
...
def main():
    ...
    for measure, patterns in temporal_patterns.items():
        ...
        for pattern_name, pattern_data in patterns.items():
            print(f"\n{pattern_name}:")
            print(pattern_data)
            pattern_data.to_frame().to_csv(output_dir / f'temporal_pattern_{measure}_{pattern_name}.csv')
    ...
    corr_matrix, grouped_correlations = analyze_feature_correlations(processed_df)
    print("\nFeature Correlations with Stress by Group:")
    for group, correlations in grouped_correlations.items():
        print(f"\n{group}:")
        print(correlations)
    
    pd.DataFrame(grouped_correlations).to_csv(output_dir / 'feature_correlations_by_group.csv')
    ...
    for feature in watch_specific_features:
        if feature in processed_df.columns:
            plt.figure(figsize=(10, 6))
            sns.scatterplot(data=processed_df, x=feature, y='stress_normalized', hue='id')
